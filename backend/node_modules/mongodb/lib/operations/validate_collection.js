    ...
    execute(server, session, callback) {
        ...
        super.executeCommand(server, session, this.command, (err, doc) => {
            if (err != null)
                return callback(err);
            if (doc.ok === 0)
                return callback(new error_1.MongoDriverError('Error with validate command'));
            if (doc.result != null && doc.result.constructor !== String)
                return callback(new error_1.MongoDriverError('Error with validation data'));
            if (doc.result != null && doc.result.match(/exception|corrupt/) != null)
                return callback(new error_1.MongoDriverError('Error: invalid collection ' + collectionName));
